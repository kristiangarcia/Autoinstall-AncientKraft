...
namespace AutoInstall
{
    public partial class Principal : Form
    {
        ...
        private async void LoadModpackVersion()
        {
            ...
            string modpackFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), ".minecraft", "modpack_version.txt");
            if (File.Exists(modpackFilePath))
            {
                modpack_version = File.ReadAllText(modpackFilePath);
                label1.Visible = true;
                label1.Text = "Modpack: v" + modpack_version;
            }
            else
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(selectedPath))
            {
                ...
                selectedPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), ".minecraft");
            }
            if (!string.IsNullOrEmpty(selectedPath))
            {
                SaveCustomPath();
        ...
        private async Task DownloadFile(string url)
        {
            ...
            string optionsPath = Path.Combine(selectedPath, "options.txt");
            if (!Directory.Exists(modsPath))
            {
                if (!File.Exists(optionsPath))
                {

                    MessageBox.Show("Por favor, inicie su Launcher en la versión Forge 1.18.2 y toque cualquier configuración gráfica, luego cierrelo e intentelo de nuevo...", "No se encontró archivo de configuración", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Environment.Exit(0);
                }
                else
                {
                    MessageBox.Show("Por favor, inicie su Launcher al menos una vez en la versión Forge 1.18.2 antes de continuar...", "Eh eh eh.. detente!!!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Environment.Exit(0);
                }
            }
            ...
            // Descargar archivo zip de dropbox
            using (HttpClient client = new HttpClient())
            using (HttpResponseMessage response = await client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead))
            using (Stream streamToReadFrom = await response.Content.ReadAsStreamAsync())
            {
                string fileToWriteTo = Path.Combine(selectedPath, "mods.zip");
            ...
            button2.Text = "Extrayendo archivos...";
            await Task.Delay(1500);

            // Extraer archivo
            using (ZipFile zip = ZipFile.Read(selectedPath + Path.DirectorySeparatorChar + "mods.zip"))
            {
            ...
            // Combinar la ruta seleccionada con el nombre del archivo options.txt
            string filePath = Path.Combine(selectedPath, "options.txt");
            ...
            // Eliminar el archivo mods.zip
            string fileToDelete = Path.Combine(selectedPath, "mods.zip");
            ...
            MessageBox.Show("La instalación del modpack oficial de InfernoLand ha finalizado. Ahora ejecute su Launcher en la versión Forge 1.18.2 (no uses Optifine)", "Instalación Completada", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void SaveModpackVersion()
        {
            try
            {
                ...
                string modpackFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), ".minecraft", "modpack_version.txt");
                File.WriteAllText(modpackFilePath, modpack_version);
            }
            catch (UnauthorizedAccessException)
            {
                MessageBox.Show("No se puede guardar la versión actual del programa.", "Error de acceso", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                // Borrar carpeta mods, config, XaeroWorldMap y Distant_Horizons_server_data
                string[] foldersToDelete = { "mods", "XaeroWorldMap", "Distant_Horizons_server_data", "config" };
                ...
                // Borrar archivos extras.txt y preset.txt
                string versionFilePath = Path.Combine(selectedPath, "version.txt");
                string extrasFilePath = Path.Combine(selectedPath, "extras.txt");
                string presetFilePath = Path.Combine(selectedPath, "preset.txt");
                string installedFilePath = Path.Combine(selectedPath, "installed.txt");
                string modpackVersionFilePath = Path.Combine(selectedPath, "modpack_version.txt");
                ...
                if (File.Exists(modpackVersionFilePath))
                {
                    File.Delete(modpackVersionFilePath);
                }
